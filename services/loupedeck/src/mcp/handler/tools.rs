...
impl MessageHandler<FfiEnvelopePayload<InvokeToolMessage>> for LoupedeckService {
    fn handle_message(&self, message: FfiEnvelopePayload<InvokeToolMessage>, _sender_id: &str) {
        let tool_name = message.0.name.to_string();
        debug!("Loupedeck Service: InvokeToolMessage name={}", tool_name);
        ...
        match tool {
            ...
            MacroPadMcpTools::StreamDeckSetBrightness => {
                let response = InvokeToolResponse::error(&message.0.correlation_id, "Tool not supported by Loupedeck service");
                broadcaster.broadcast_message_to_topic(response);
            }
        }
    }
