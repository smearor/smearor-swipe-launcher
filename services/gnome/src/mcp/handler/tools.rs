use crate::service::GnomeCommand;
...
impl MessageHandler<FfiEnvelopePayload<InvokeToolMessage>> for GnomeWorkspaceService {
    fn handle_message(&self, message: FfiEnvelopePayload<InvokeToolMessage>, _sender_id: &str) {
        let tool_name = message.0.name.to_string();
        trace!("GNOME Service: InvokeToolMessage name={}", tool_name);
        ...
        match tool {
            CompositorMcpTools::SwitchWorkspace => {
                ...
                let _ = self.command_sender.send(GnomeCommand::SwitchWorkspace(SwitchWorkspaceMessage {
