...
*SSE (Server-Sent Events)** using the `rust-mcp-sdk` and `rust-mcp-axum` crates.
...
queries this registry and exposes the tools dynamically.
...
core does not respond in time, the server returns a JSON-RPC error with `JSONRPC_INTERNAL_ERROR` and a clear timeout message, instead of leaving the client
...
The resource `area://list` is read directly from the `AreaManager` and is the only core resource currently implemented. It is registered as a built-in resource
in `mcp-server/src/resources.rs`. An area status entry contains at least:
...
3. When a tool is called, the host sends an `InvokeToolMessage` to the owning plugin via `TOPIC_MCP_INVOKE_TOOL` with a correlation ID and JSON-encoded
   arguments.
4. The plugin handles `InvokeToolMessage` via `MessageHandler<FfiEnvelopePayload<InvokeToolMessage>>`, executes the action (e.g., `PowerCommand::Execute`), and
   responds with an `InvokeToolResponse` broadcast to `TOPIC_MCP_TOOL_RESPONSE`.
...
* **MCP Prompts support:**
    * Model types: `RegisterPromptMessage`, `InvokePromptMessage`, `InvokePromptResponse`, `PromptMessage` in `model/mcp`.
    * Topics: `mcp.register.prompt`, `mcp.invoke.prompt`, `mcp.prompt.response`.
    * Core prompts: `launcher_overview`, `area_control_help`, `broker_message_guide` in `mcp-server/src/prompts.rs`.
    * Plugin-Prompt-Registry: plugins register prompts via `RegisterPromptMessage`, host routes them to `McpRegistry`.
    * Prompt invocation: `InvokePromptMessage` dispatched to plugins, responses via `InvokePromptResponse` and `McpResponseTracker`.
    * `notifications/prompts/list_changed` notification pushed over SSE when prompts are registered.
    * Implemented service plugin prompts:
        * `services/weather`: `weather_query_guide`.
        * `services/mpris`: `mpris_control_guide`.
        * `services/terminal_command`: `terminal_command_guide`.
        * `services/app-launcher`: `app_launch_guide`.
        * `services/power`: `power_action_guide`.
        * `services/voice_assistant`: `voice_assistant_status`, `memory_guide`.
    * Voice assistant integration: prompt catalog tracking, context injection from registered prompts, system prompt extension.
...
* Model types: `RegisterPromptMessage`, `InvokePromptMessage`, `InvokePromptResponse`, `PromptMessage` with topics in `model/mcp`.
* Core prompts module `mcp-server/src/prompts.rs`: `launcher_overview`, `area_control_help`, `broker_message_guide`.
* Plugin-Prompt-Registry: plugins register prompts via `RegisterPromptMessage` to `mcp.register.prompt` topic.
* Host routing: `TOPIC_MCP_REGISTER_PROMPT` → `McpRegistry`, `TOPIC_MCP_INVOKE_PROMPT` → plugin dispatch, `TOPIC_MCP_PROMPT_RESPONSE` → `McpResponseTracker`.
* `notifications/prompts/list_changed` notification pushed over SSE.
* Voice assistant integration: prompt catalog, context injection, system prompt extension.
* Service plugin prompts implemented:
    * `services/weather`: `weather_query_guide` (dynamic location context).
    * `services/mpris`: `mpris_control_guide` (player list and control tools).
    * `services/terminal_command`: `terminal_command_guide` (configured/running commands).
    * `services/app-launcher`: `app_launch_guide` (running apps, search→launch pipeline).
    * `services/power`: `power_action_guide` (capabilities, inhibitors, safety instructions).
    * `services/voice_assistant`: `voice_assistant_status` (state, transcript, answer), `memory_guide` (memory management instructions).
