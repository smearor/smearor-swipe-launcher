...
the message fields directly from memory:
...
signature. This type alias already implies `Send + Sync`; the compiler rejects non-thread-safe futures. /// NOTE: `stabby::compiler::Send` does not exist in
stabby 72.1.8. Stabby uses separate type aliases /// (`DynFuture`, `DynFutureUnsync`, `DynFutureUnsend`) rather than generic Send bounds.
