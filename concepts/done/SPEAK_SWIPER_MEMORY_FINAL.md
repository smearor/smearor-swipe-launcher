...
| `services/voice_assistant/src/service/loaded_service.rs` | Replace `llm_engine` with `llm_worker`. Add `conversation_history`, `entity_store`, `entity_db`, `semantic_memory`, `tool_router` fields. Initialize in `new()`. Reconstruct entity store from SQLite on startup. |
