...
Use cases include fetching weather data from online services and querying or controlling IoT devices in the local network. Concrete examples include controlling
Shelly devices such as turning a light on or off via a Shelly 1, changing the color of an RGBW stripe via a Shelly RGBW2, or toggling a fan via a Shelly
...
Widget or a dedicated future widget.
...
data. The HTTP service can fetch this data from free APIs and publish it on dedicated topics, so a future weather/astronomy widget can render it in the same
visual style.
...
response on the configured `response_topic`.
