...
tracing events → registry()
  ...
  └── LogBufferLayer → LogBuffer (Arc<Mutex<VecDeque<LogEntry>>>)
                              │
                              ▼
...
- `query(min_level, target_prefix, since_ms, limit) -> Vec<LogEntry>` — filtered retrieval
- `clear()` — empty the buffer
- `len() -> usize` — current entry count

### 3.3 LogBufferLayer

Implements `tracing_subscriber::Layer`. `on_event` extracts metadata (level, target, file, line) and visited message, creates `LogEntry`, pushes to buffer.

Uses a `LogEntryVisitor` implementing `tracing::field::Visit` to extract the formatted message from event fields.
...
if params.name == "launcher_get_logs" {
    let log_params: GetLogsParams = serde_json::from_value(
...
|--------------------------|--------------------------------------------------------------------|
| `mcp-server`             | `tracing-subscriber` (workspace), `serde`, `serde_json` (existing) |
...
- [ ] Mutex poisoning handled gracefully (return empty on poison)

---

## 10. Common Pitfalls

- **Mutex poisoning**: If a thread panics while holding the `LogBuffer` mutex, subsequent locks will be poisoned. Use `unwrap_or_else(|e| e.into_inner())` to
  recover gracefully.
...
- **Thread safety**: `LogBufferLayer` must be `Send + Sync`. `Arc<LogBuffer>` with `Mutex<VecDeque>` satisfies this.
