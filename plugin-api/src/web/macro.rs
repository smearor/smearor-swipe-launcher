/// Macro to generate FFI boilerplate for widget plugins that implement `WebRenderer`.
///
/// This is identical to `widget_plugin!` but also generates a `render_html` FFI wrapper
/// and sets it in the VTable. Use this for widgets that can render to HTML for web instances.
///
/// # Requirements
///
...
///
/// ```rust
/// use smearor_swipe_launcher_plugin_api::widget_plugin_web;
///
/// widget_plugin_web!(MyWidget);
/// ```
#[macro_export]
macro_rules! widget_plugin_web {
    ($widget_type:ty) => {
        paste::paste! {
