...
## Phase 6: MCP Server & AI Integration

*Focus: Exposing the launcher through the Model Context Protocol for external AI clients.*

### 🏁 Milestone 6.1: MCP Server Foundation ✅

* [x] **MCP Server Crate:** Dedicated `mcp-server` crate using `rust-mcp-sdk` and `rust-mcp-axum` for Streamable HTTP + SSE transport.
* [x] **Core Tools:** `open_area`, `close_area`, `list_areas`, `list_all_areas`, `focus_area`, `send_message`, `toggle_area`, `get_area_config`,
  `open_transient_area`.
* [x] **Plugin-Tool-Registry & Plugin-Resource-Registry:** Dynamic registration of tools and resources by plugins via message broker topics.
* [x] **Service Plugin Tools & Resources:** Implemented across `power`, `network`, `wallpaper`, `sysinfo`, `weather`, `audio`, `mpris`, `app-launcher`,
  `clock`, `weather` widget.

### 🏁 Milestone 6.2: MCP Prompts ✅

* [x] **Model Types:** `RegisterPromptMessage`, `InvokePromptMessage`, `InvokePromptResponse`, `PromptMessage` in `model/mcp` with topics
  `mcp.register.prompt`, `mcp.invoke.prompt`, `mcp.prompt.response`.
* [x] **Core Prompts:** `launcher_overview`, `area_control_help`, `broker_message_guide` in `mcp-server/src/prompts.rs`.
* [x] **Plugin-Prompt-Registry:** Plugins register prompts via `RegisterPromptMessage`, host routes to `McpRegistry`, `notifications/prompts/list_changed`
  pushed over SSE.
* [x] **Prompt Invocation:** `InvokePromptMessage` dispatched to plugins, responses via `InvokePromptResponse` and `McpResponseTracker`.
* [x] **Voice Assistant Integration:** Prompt catalog tracking, context injection from registered prompts, system prompt extension.
* [x] **Service Plugin Prompts:**
