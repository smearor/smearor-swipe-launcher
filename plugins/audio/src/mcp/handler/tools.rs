...
use smearor_audio_model::AudioWidgetAction;
...
impl MessageHandler<FfiEnvelopePayload<InvokeToolMessage>> for AudioWidget {
    fn handle_message(&self, message: FfiEnvelopePayload<InvokeToolMessage>, _sender_id: &str) {
        ...
        trace!("AudioWidget: InvokeToolMessage name={} args={}", tool_name, arguments);
        ...
        if tool_name == own_button_name {
            ...
            if let Ok(widget_action) = AudioWidgetAction::from_str(&action_str) {
                match widget_action {
                    AudioWidgetAction::Expand => {
