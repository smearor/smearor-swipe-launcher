...
use smearor_mpris_model::MprisWidgetAction;
...
impl MessageHandler<FfiEnvelopePayload<InvokeToolMessage>> for MprisWidget {
    fn handle_message(&self, message: FfiEnvelopePayload<InvokeToolMessage>, _sender_id: &str) {
        ...
        trace!("MprisWidget: InvokeToolMessage name={} args={}", tool_name, arguments);
        ...
        if tool_name == own_button_name {
            ...
            if let Ok(widget_action) = MprisWidgetAction::from_str(&action_str) {
                match widget_action {
                    MprisWidgetAction::Expand => {
