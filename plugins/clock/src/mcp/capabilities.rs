...
impl McpCapabilitiesRegistrator for ClockWidget {
    fn register_mcp_capabilities(&self) {
        ...
        let tool = RegisterToolMessage::new(
            "get_current_time",
            self.config.metadata.description().unwrap_or("Returns the current local time as a structured JSON object with fields: timestamp_iso (ISO 8601), date (DD.MM.YYYY), time (HH:MM), timezone, timezone_label (CET/CEST/etc), is_summer_time (bool), day_of_week, time_of_day_context (late_night/early_morning/morning/noon/afternoon/evening), workday_status (mid_week/weekend)."),
            &schema,
        )
