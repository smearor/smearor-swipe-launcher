use crate::cpu::config::CpuWidgetConfig;
...
impl MessageHandler<FfiEnvelopePayload<PersonalizationStatusMessage>> for CpuWidget {
...
impl WidgetPlugin for CpuWidget {
...
impl WidgetBuilder for CpuWidget {
    fn build_widget(&mut self) -> Widget {
        ...
        if let Some(ref temp_label) = temperature_label {
            apply_text_color(temp_label, self.config.percentage.text_colors.info_text_color());
        }
