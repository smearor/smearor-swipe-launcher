use crate::battery::config::BatteryWidgetConfig;
...
pub struct BatteryWidget {
    ...
    pub latest_status: Rc<RefCell<Option<BatteryStatusMessage>>>,
...
impl MessageHandler<FfiEnvelopePayload<PersonalizationStatusMessage>> for BatteryWidget {
...
impl AcceptTopic<FfiEnvelope> for BatteryWidget {
    fn accept_topic(&self, topic: &str) -> bool {
        topic == TOPIC_BATTERY || topic == <FfiEnvelopePayload<PersonalizationStatusMessage> as MessageTopic>::topic()
    }
}
...
impl WidgetPlugin for BatteryWidget {
...
impl WidgetBuilder for BatteryWidget {
    fn build_widget(&mut self) -> Widget {
        ...
        if let Some(ref label) = percentage_widget.value_label {
            ...
            apply_text_color(label, self.config.percentage.text_colors.main_text_color());
        }

        if let Some(ref status) = status_label {
            apply_text_color(status, self.config.percentage.text_colors.info_text_color());
        }
