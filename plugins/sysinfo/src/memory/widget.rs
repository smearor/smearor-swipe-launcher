use crate::memory::config::MemoryWidgetConfig;
...
pub struct MemoryWidget {
    ...
    pub latest_status: Rc<RefCell<Option<MemoryStatusMessage>>>,
...
impl MessageHandler<FfiEnvelopePayload<PersonalizationStatusMessage>> for MemoryWidget {
...
impl AcceptTopic<FfiEnvelope> for MemoryWidget {
    fn accept_topic(&self, topic: &str) -> bool {
        topic == TOPIC_MEMORY || topic == <FfiEnvelopePayload<PersonalizationStatusMessage> as MessageTopic>::topic()
    }
}
...
impl WidgetPlugin for MemoryWidget {
...
impl WidgetBuilder for MemoryWidget {
    fn build_widget(&mut self) -> Widget {
        ...
        if let Some(ref details) = details_label {
            apply_text_color(details, self.config.percentage.text_colors.info_text_color());
        }
