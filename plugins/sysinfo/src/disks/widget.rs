...
use crate::disks::config::DiskDisplayMode;
use crate::disks::config::DisksWidgetConfig;
...
pub struct DisksWidget {
    ...
    pub latest_status: Rc<RefCell<Option<DisksStatusMessage>>>,
...
impl MessageHandler<FfiEnvelopePayload<PersonalizationStatusMessage>> for DisksWidget {
...
impl AcceptTopic<FfiEnvelope> for DisksWidget {
    fn accept_topic(&self, topic: &str) -> bool {
        topic == TOPIC_DISKS || topic == <FfiEnvelopePayload<PersonalizationStatusMessage> as MessageTopic>::topic()
    }
}
...
impl WidgetPlugin for DisksWidget {
...
impl WidgetBuilder for DisksWidget {
    fn build_widget(&mut self) -> Widget {
        ...
        apply_text_color(&placeholder, self.config.text_colors.info_text_color());
...
impl DisksWidget {
    fn build_gauge_widget(&mut self) -> Widget {
        let percentage_config = PercentageWidgetConfig {
            ...
            text_colors: self.config.text_colors.clone(),
        };
