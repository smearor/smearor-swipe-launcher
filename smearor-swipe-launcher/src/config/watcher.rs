...
pub struct ConfigWatcher {
    ...
    path_to_instance: HashMap<PathBuf, String>,
    /// Maps instance ID to the main config path (used for reload).
    instance_to_config: HashMap<String, PathBuf>,
...
impl ConfigWatcher {
    ...
    pub fn new() -> Self {
        Self {
            path_to_instance: HashMap::new(),
            instance_to_config: HashMap::new(),
    ...
    pub fn add_config(&mut self, config_path: &Path, instance_id: &str, include_paths: &[PathBuf]) {
    ...
    fn store_hash(&mut self, path: &Path) {
    ...
    pub fn start(self) -> Receiver<ConfigReloadRequest> {
        ...
        for path in self.path_to_instance.keys() {
        ...
        let path_to_instance = Arc::new(self.path_to_instance);
        let instance_to_config = Arc::new(self.instance_to_config);
        let file_hashes = Arc::new(Mutex::new(self.file_hashes));
                            ...
                            if let Some(instance_id) = path_to_instance.get(&path) {
                                if let Some(config_path) = instance_to_config.get(instance_id) {
                                    if has_content_changed(&file_hashes, &path) {
                                        pending = Some((instance_id.clone(), config_path.clone()));
                                    }
                                } else {
...
fn has_content_changed(file_hashes: &Arc<Mutex<HashMap<PathBuf, u64>>>, path: &Path) -> bool {
