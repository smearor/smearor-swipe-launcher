...
pub fn start_mcp_server(host: &LauncherHost, services_config: &ServicesConfig, log_buffer: Option<Arc<LogBuffer>>) -> McpServerHandles {
    ...
    let mut server = McpServer::new(mcp_config, host.mcp_registry.clone(), mcp_command_sender.clone(), log_buffer);
