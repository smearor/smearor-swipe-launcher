...
use crate::mcp::plugin_invoke::invoke_plugin_tool_sender;
use smearor_mcp_server::CommandResponseWrapper;
use smearor_mcp_server::InvokePluginToolParams;
use smearor_swipe_launcher_plugin_api::FfiEnvelope;
use tokio::sync::mpsc::UnboundedSender;

/// Handle a plugin tool invocation by sending the request via the broker
/// and awaiting the response with a 10-second timeout.
pub(crate) async fn handle_plugin_tool(
    broker_sender: &UnboundedSender<FfiEnvelope>,
    response_tracker: &McpResponseTracker,
    command: CommandResponseWrapper<InvokePluginToolParams>,
) {
    let receiver = response_tracker.register(command.params.correlation_id.clone());
    let send_result = invoke_plugin_tool_sender(
