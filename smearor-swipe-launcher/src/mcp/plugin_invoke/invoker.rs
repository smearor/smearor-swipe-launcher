...
fn invoke_plugin<T>(broker_sender: &UnboundedSender<FfiEnvelope>, message: T, label: &'static str) -> Result<(), PluginInvokeError>
where
...
pub fn invoke_plugin_resource_sender(broker_sender: &UnboundedSender<FfiEnvelope>, uri: &str, correlation_id: &str) -> Result<(), PluginInvokeError> {
    invoke_plugin(broker_sender, InvokeResourceMessage::new(uri, correlation_id), "resource")
}
